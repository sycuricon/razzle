...
class TransManager(SectionManager):
    ...
    def _generate_sections(self):
        ...
        trap_section = self.section[".trap"] = FuzzSection(
            ".trap", Flag.X | Flag.R | Flag.W
        )
    ...
    def _distribute_address(self):
        self.section[".trap"].get_bound(
            self.memory_bound[0][0], self.memory_bound[0][0], 0x1000
        )
        ...
        self.section[".data"].get_bound(
            self.virtual_memory_bound[0][0] + 0x2000,
            self.memory_bound[0][0] + 0x2000,
            0x1000,
        )
        self.section[".poc"].get_bound(
            self.virtual_memory_bound[1][0], self.memory_bound[1][0], 0x1000
        )
