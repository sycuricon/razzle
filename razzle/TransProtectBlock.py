...
class SecretProtectBlock(TransBlock):
    def __init__(self, extension, output_path, pmp_r, pmp_l, pte_r, pte_v, train_priv, train_addr, attack_priv, attack_addr):
    ...
    def gen_instr(self):
        ...
        if self.pte_r == False or self.pte_v == False:
            vaddr_label = 'vaddr_0xfffffffffff04000_paddr_0x80004000'\
                if self.train_priv == 'S' and self.train_addr == 'v' or self.attack_priv == 'S' and self.train_addr == 'p' else\
...
class TransProtectManager(TransBaseManager):
    ...
    def gen_block(self, config):
        ...
        self.secret_protect_block = SecretProtectBlock(self.extension, self.output_path, \
            config['pmp_l'], config['pmp_r'], config['pte_r'], config['pte_v'], config['train_priv'],  \
            config['train_addr'], config['attack_priv'], config['attack_addr'])
